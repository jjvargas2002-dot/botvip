...
@app.route("/admin/migrar_supabase")
def admin_migrar_supabase():
    token = request.args.get("token")
    if token != "migrar_botvip_2026":
        return jsonify({"status": "error", "message": "Acceso no autorizado"}), 403
        
    conn_str = request.args.get("conn_str")
    if not conn_str:
        return jsonify({"status": "error", "message": "Falta parámetro conn_str"}), 400
        
    import sqlalchemy
    from sqlalchemy.orm import sessionmaker
    from models import Operador, Cliente, Caso, Averia, StockBranch, Box
    
    try:
        # Intentar conectar con la nueva DB
        engine_sb = sqlalchemy.create_engine(conn_str, connect_args={"connect_timeout": 15})
        SessionSB = sessionmaker(bind=engine_sb)
        session_sb = SessionSB()
        
        # Probar conexión
        session_sb.execute(sqlalchemy.text("SELECT 1"))
        
        # 1. Crear las tablas en la nueva DB
        db.metadata.create_all(bind=engine_sb)
        
        # 2. Copiar operadores
        operadores = Operador.query.all()
        session_sb.execute(sqlalchemy.text("TRUNCATE TABLE operadores CASCADE"))
        for op in operadores:
            new_op = Operador(
                id=op.id, nombre=op.nombre, dni=op.dni, correo=op.correo,
                password_hash=op.password_hash, rol=op.rol, branch=op.branch,
        ...
        session_sb.execute(sqlalchemy.text("TRUNCATE TABLE clientes CASCADE"))
        for cl in clientes:
            new_cl = Cliente(
                id=cl.id, codigo_cliente=cl.codigo_cliente, telefono=cl.telefono,
                nombre=cl.nombre, numero_referencia=cl.numero_referencia,
        ...
        session_sb.execute(sqlalchemy.text("TRUNCATE TABLE casos CASCADE"))
        for cs in casos:
            new_cs = Caso(
                id=cs.id, cliente_id=cs.cliente_id, operador_id=cs.operador_id,
                tipo=cs.tipo, estado=cs.estado, problema=cs.problema,
                fecha_disponible=cs.fecha_disponible, hora_disponible=cs.hora_disponible,
                tecnico_nombre=cs.tecnico_nombre, tecnico_telefono=cs.tecnico_telefono,
                tecnico_estado=cs.tecnico_estado, fecha_creacion=cs.fecha_creacion,
        ...
        session_sb.execute(sqlalchemy.text("TRUNCATE TABLE averias CASCADE"))
        for av in averias:
            new_av = Averia(
                id=av.id, branch=av.branch, codigo_wo=av.codigo_wo, cuenta=av.cuenta,
                detalles=av.detalles, dias_pendientes=av.dias_pendientes, estado=av.estado,
                status_caja=av.status_caja, contrata=av.contrata, periodo_pendiente=av.periodo_pendiente,
                site=av.site, caja=av.caja, coordenadas=av.coordenadas, origen=av.origen,
                fecha_creacion=av.fecha_creacion, fecha_resolucion=av.fecha_resolucion,
                tecnico_id=av.tecnico_id, material_cable_m=av.material_cable_m,
                material_conectores=av.material_conectores, material_rosetas=av.material_rosetas,
                material_mangas=av.material_mangas, material_acopladores=av.material_acopladores,
        ...
        session_sb.execute(sqlalchemy.text("TRUNCATE TABLE stock_branch CASCADE"))
        for st in stocks:
            new_st = StockBranch(
                id=st.id, branch=st.branch, material_codigo=st.material_codigo,
                material_nombre=st.material_nombre, stock_actual=st.stock_actual,
                stock_enviado_noc=st.stock_enviado_noc, fecha_envio_noc=st.fecha_envio_noc,
        ...
        tablas_id = ["operadores", "clientes", "casos", "averias", "stock_branch", "boxes"]
        for t in tablas_id:
            query_seq = f"SELECT setval(pg_get_serial_sequence('{t}', 'id'), COALESCE(max(id), 1)) FROM {t}"
            session_sb.execute(sqlalchemy.text(query_seq))
        session_sb.commit()
        ...
        if 'session_sb' in locals():
            session_sb.rollback()
        return jsonify({"status": "error", "message": f"Error de conexión o escritura: {str(e)}"}), 500
    finally:
        if 'session_sb' in locals():
            session_sb.close()
...
def agrupar_clientes_averia(id):
    ...
    # 2. Si la avería local está PENDIENTE o REPARADA, permitir seleccionar el principal reparado de los últimos 30 días (antes o después)
    manage_principal = request.args.get("manage_principal") == "true"
    
    if not manage_principal:
        ...
        validos = []
        now = datetime.now()
        ref_av = averia.fecha_resolucion or averia.fecha_creacion
        ref_av_clean = ref_av.replace(tzinfo=None) if ref_av else None
        ...
        for p in principales:
            ...
            in_last_30_days_ref = False
            if ref_av_clean:
                in_last_30_days_ref = abs((ref_p - ref_av_clean).days) <= 30
            else:
                in_last_30_days_ref = True
                
            if in_last_30_days_now or in_last_30_days_ref:
                validos.append(p)
                
        if averia.estado == "PENDIENTE" and not validos:
            flash(f"No hay ninguna avería reparada (principal) en el SITE '{site}' para agrupar. Por favor resuelve el ticket principal primero.", "warning")
            return redirect(url_for("dashboard"))
            
        if validos and (averia.estado == "PENDIENTE" or averia.estado == "REPARADO"):
            if request.method == "POST":
                selected_principal_id = request.form.get("principal_id")
                if selected_principal_id:
                    principal = db.session.get(Averia, int(selected_principal_id))
                    if principal:
                        old_list = [c.strip() for c in (principal.cuentas_asociadas or "").split(",") if c.strip()]
                        if averia.cuenta and averia.cuenta not in old_list:
                        ...
                        try:
                            db.session.commit()
                            flash(f"Avería {averia.cuenta} agrupada con éxito bajo la avería principal {principal.cuenta}.", "success")
                        except Exception as e:
                            db.session.rollback()
                            flash(f"Error al agrupar automáticamente: {str(e)}", "danger")
